...
private constructor(
  ...
  companion object {
    ...
    @JvmStatic @JvmName("parse") fun Input.toManifest(): AndroidManifest = toUtf8().toManifest()
    ...
    private fun Document.normalizeWhitespace() {
      ...
      val emptyNodes =
        XPathFactory.newInstance()
          .newXPath()
          .evaluate("//text()[normalize-space()='']", this, XPathConstants.NODESET) as NodeList
    ...
    private fun Document.toManifest(): AndroidManifest {
      ...
      require(manifestElement.tagName == "manifest") { "Unable to find root <manifest> tag" }
      ...
      val versionCodeMajor =
        manifestElement.getAttributeOrNull(ANDROID_NS, "versionCodeMajor")?.toInt() ?: 0
      val versionCode =
        if (versionCodeMinor != null) {
    ...
    private operator fun NodeList.iterator() =
    ...
    private operator fun NamedNodeMap.iterator() =
      object : Iterator<Node> {
        private var index = 0
