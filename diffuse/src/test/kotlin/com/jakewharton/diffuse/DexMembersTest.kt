...
class DexMembersTest {
  ...
  @Test
  fun fieldTypeLambdaNumberRemoved() {
    val field = Field(TypeDescriptor("Lcom/example/Foo$\$Lambda$26;"), "bar", barDescriptor)
    val fieldWithoutLambdaNumber =
      Field(TypeDescriptor("Lcom/example/Foo$\$Lambda;"), "bar", barDescriptor)
    assertThat(field.withoutSyntheticSuffix()).isEqualTo(fieldWithoutLambdaNumber)
  }

  @Test
  fun methodNameSyntheticNumberRemoved() {
  ...
  @Test
  fun methodNameLambdaNumberRemoved() {
    val method =
      Method(
        fooDescriptor,
    ...
    val methodWithoutLambdaNumber =
      Method(fooDescriptor, "lambda\$refreshSessionToken\$MainActivity", emptyList(), barDescriptor)
    assertThat(method.withoutSyntheticSuffix()).isEqualTo(methodWithoutLambdaNumber)
  }

  @Test
  fun methodTypeLambdaNumberRemoved() {
    val method =
      Method(TypeDescriptor("Lcom/example/Foo$\$Lambda$26;"), "bar", emptyList(), barDescriptor)
    val methodWithoutLambdaNumber =
      Method(TypeDescriptor("Lcom/example/Foo$\$Lambda;"), "bar", emptyList(), barDescriptor)
    assertThat(method.withoutSyntheticSuffix()).isEqualTo(methodWithoutLambdaNumber)
  }
}
