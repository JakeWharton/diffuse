...
class DexMethodTest {
  ...
  @Test
  fun renderKotlinLambdaFunctionName() {
    val synthetic =
      Method(
        fooDescriptor,
        "lambda\$refreshSessionToken$14\$MainActivity",
        emptyList(),
        barDescriptor,
      )
    assertThat(synthetic.toString())
      .isEqualTo("com.example.Foo lambda\$refreshSessionToken$14\$MainActivity() → Bar")
    val removedSynthetic = synthetic.withoutSyntheticSuffix()
    assertThat(removedSynthetic.toString())
      .isEqualTo("com.example.Foo lambda\$refreshSessionToken\$MainActivity() → Bar")
  }

  @Test
  fun renderKotlinLambdaFunctionNameNumberLast() {
    val synthetic =
      Method(fooDescriptor, "lambda\$refreshSessionToken$14", emptyList(), barDescriptor)
    assertThat(synthetic.toString())
      .isEqualTo("com.example.Foo lambda\$refreshSessionToken$14() → Bar")
    val removedSynthetic = synthetic.withoutSyntheticSuffix()
    assertThat(removedSynthetic.toString())
      .isEqualTo("com.example.Foo lambda\$refreshSessionToken() → Bar")
  }

  @Test
  fun renderKotlinInvalidLambdaName() {
  ...
  fun renderKotlinLambdaClassName() {
    val synthetic =
      Method(TypeDescriptor("Lcom/example/Foo$\$Lambda$26;"), "bar", emptyList(), barDescriptor)
  ...
  @Test
  fun compareToDifferentParameterList() {
    val one = Method(fooDescriptor, "bar", emptyList(), barDescriptor)
  ...
  @Test
  fun compareToDifferentReturnType() {
    val one = Method(fooDescriptor, "bar", emptyList(), barDescriptor)
