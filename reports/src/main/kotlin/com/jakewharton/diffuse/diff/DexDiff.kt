...
internal class DexDiff(val oldDexes: List<Dex>, val newDexes: List<Dex>) : BinaryDiff {
  ...
  val declaredMethods =
    componentDiff(oldDexes, newDexes) { it.declaredMembers.filterIsInstance<Method>() }
  val referencedMethods =
    componentDiff(oldDexes, newDexes) { it.referencedMembers.filterIsInstance<Method>() }
  val fields = componentDiff(oldDexes, newDexes) { it.members.filterIsInstance<Field>() }
  val declaredFields =
    componentDiff(oldDexes, newDexes) { it.declaredMembers.filterIsInstance<Field>() }
